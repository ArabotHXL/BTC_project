...
async function main() {
  ...
  try {
    ...
    const dealId = dealRes.data.deal ? dealRes.data.deal.id : dealRes.data.id;
    ...
    if (automationLogs.length > 0) {
      results.push({
        ...
        message: `Found ${automationLogs.length} automation logs for test entities`
      });
    } else {
      results.push({
        test: 'Automation Execution', 
        status: 'FAIL',
        reason: 'No automation logs found for test entities - automation rules did not execute',
        logCount: 0
      });
    }
