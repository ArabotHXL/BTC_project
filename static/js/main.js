...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function calculateAndDisplayROI(data) {
        ...
        try {
            ...
            var minerCount = data.inputs ? (data.inputs.miner_count || 1) : 1;
            var minerModel = data.inputs ? (data.inputs.miner_model || '') : '';
            var perMinerCost = 8000; // 美元/台
            var infrastructureCost = minerCount * 500; // 基础设施成本估算
            var capex = (minerCount * perMinerCost) + infrastructureCost;
            ...
            var annualRoi = (annualProfit / capex) * 100;
            
            // 计算回本周期（月）
            var paybackPeriodMonths = monthlyProfit > 0 ? capex / monthlyProfit : Infinity;
            ...
            if (paybackPeriodEl) {
                if (monthlyProfit <= 0) {
                    ...
                    var years = Math.floor(paybackPeriodMonths / 12);
                    var months = Math.round(paybackPeriodMonths % 12);
                    var displayText = '';
