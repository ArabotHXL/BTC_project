...
DEFAULT_NETWORK_DIFFICULTY = 79.8 * 10**12  # 79.8T (default value)
DEFAULT_BTC_PRICE = 80000  # Default BTC price in USD
DEFAULT_NETWORK_HASHRATE = 700 * 10**6  # 700 EH/s in TH/s
BLOCK_REWARD = 3.125  # Current BTC block reward (after the April 2024 halving)
...
def get_real_time_btc_price():
    ...
    try:
        ...
        logging.error(f"Failed to get real-time BTC price: {e}")
        return DEFAULT_BTC_PRICE

def get_real_time_btc_hashrate():
    """Get the current Bitcoin network hashrate from blockchain.info"""
    try:
        response = requests.get('https://blockchain.info/q/hashrate')
        if response.status_code == 200:
            hashrate_th = float(response.text.strip())  # Original data in TH/s
            return hashrate_th  # Keep in TH/s for consistency
        else:
            raise Exception("API response error")
    except Exception as e:
        logging.error(f"Failed to get real-time BTC hashrate: {e}")
        return DEFAULT_NETWORK_HASHRATE

def get_real_time_difficulty():
    """Get the current Bitcoin mining difficulty from blockchain.info"""
    try:
        ...
        logging.error(f"Failed to get real-time BTC difficulty: {e}")
        return DEFAULT_NETWORK_DIFFICULTY

def get_real_time_block_reward():
    """Get the current block reward based on the current block height"""
    try:
        response = requests.get('https://blockchain.info/q/getblockcount')
        if response.status_code == 200:
            block_height = int(response.text.strip())
            # Determine block reward based on halving schedule
            if block_height >= 840000:  # April 2024 halving
                block_reward = 3.125
            elif block_height >= 630000:  # May 2020 halving
                block_reward = 6.25
            elif block_height >= 420000:  # July 2016 halving
                block_reward = 12.5
            elif block_height >= 210000:  # November 2012 halving
                block_reward = 25.0
            else:
                block_reward = 50.0
            return block_reward
