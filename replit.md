...
- **Skills Framework**: Unified capability plugin system providing standardized tool interfaces for AI closed-loop automation and human operators. 5 P0 skills (telemetry_snapshot, alert_triage, rca_quick_diagnose, ticket_draft, curtailment_dry_run) with built-in RBAC, audit trail, rate limiting, and feature flags. API: GET/POST `/api/skills[/<id>][/run]`. See `.agents/skills/skills-framework/SKILL.md` for details.
