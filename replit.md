...
- **Curtailment APIs** (modules/hosting/routes.py): 9 production-ready endpoints - /calculate (compute optimal plan), /execute (create and run plan), /cancel (abort plan), /emergency-restore (recover all miners), /strategies (list site strategies), /kpis (real-time statistics), /history (execution audit trail), /sites/list (dropdown data), /predict (AI-powered 24h optimal schedule). All with comprehensive parameter validation returning 400 for malformed inputs
...
- **AI Prediction System** (intelligence/curtailment_predictor.py): ARIMA-based 24-hour forecasting integrating BTC price prediction (intelligence/forecast.py), difficulty forecasting, and power pricing optimization. Calculates hourly economic impact (electricity cost saved vs. mining revenue lost), generates net benefit curve, identifies optimal curtailment hours based on profitability analysis. Supports all 5 pricing modes (fixed, peak-valley, hourly-24, API real-time, monthly contract). UI features Chart.js v4.4.0 visualization of 24-hour forecast, summary statistics (hours to curtail, cost saved, revenue lost, net benefit), and one-click "Apply AI Recommendation" workflow to auto-populate curtailment planner inputs
- **UI Integration**: Dedicated curtailment management page at /hosting/host/curtailment with KPI dashboard (total miners, curtailed count, power saved), AI Smart Prediction panel with 24-hour chart visualization, scenario planner wizard, execution history table, emergency restore button. Bilingual support (EN/中文), responsive design with hosting theme
- **Status**: All core functionality operational and e2e tested (database schema, performance scorer, strategy engine, APIs, executor, UI, AI prediction with Chart.js visualization)
...
- **Smart Power Curtailment Module**: AI-powered intelligent load management system for mining farm operators. Core workflow: (1) Performance Scoring - continuous evaluation of all miners using weighted metrics (hashrate efficiency 70%, power efficiency 20%, uptime 10%), (2) Strategy Selection - choose from 4 algorithms (pure performance, customer priority with VIP protection, fair distribution, custom rules), (3) AI Smart Prediction - ARIMA-based 24-hour forecasting with Chart.js visualization showing optimal curtailment hours based on BTC price, difficulty, and electricity pricing (supports 5 modes: fixed, peak-valley, hourly-24, API real-time, monthly contract), (4) Scenario Planning - input target power reduction to receive optimized shutdown recommendations with economic impact analysis (revenue lost vs. cost saved), (5) Execution - deploy plans in auto/semi-auto/manual modes with comprehensive audit trails, (6) Emergency Recovery - one-click restore all curtailed miners. Key features: support for 5000+ miner operations, batch SQL optimization, CGMiner API integration (Plan-B fallback to status updates), transaction-safe rollback protection, real-time KPI dashboard, execution history with full audit trails, bilingual UI (/hosting/host/curtailment). Role-based access control: mining_site_owner/admin can create and execute plans, operators can execute pre-approved plans. Database-driven with 6 models and 8 enums ensuring referential integrity and comprehensive state management.
...
- **Chart.js v3 & v4.4.0**: Data visualization (v3 for general dashboards, v4.4.0 for AI prediction charts).
