...
- **Database-Driven Miner Selection Integration Complete (Aug 22)**: Fixed architectural inconsistency where batch calculator used hardcoded JavaScript miner data instead of database records. Now dynamically loads all miner models from miner_models table with hashrate, power consumption, price, manufacturer, and efficiency data. Batch calculator interface automatically reflects database updates, ensuring data consistency across the platform. Eliminates maintenance overhead of managing dual data sources.
