...
## Changelog  
- July 2, 2025: **COMPLETED** - System timezone optimization with local time display: implemented EST timezone for backend data recording while displaying user-friendly local system time in frontend charts; modified updatePriceChart function to show month-day-hour-minute format (月日时分) using JavaScript toLocaleString() for chart labels and report timestamps; backend maintains EST data consistency while users see familiar local time presentation
